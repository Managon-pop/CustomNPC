...
class CustomNPC extends PluginBase implements Listener{
	...
	public function onJoin(PlayerJoinEvent $event)
	{
		...
		{
			if(!$data["show"]) return;//show equals false
			$type = $data["type"];
			$x = $data["x"];
			$y = $data["y"];
			$z = $data["z"];
			...
			if($type === "player")
			{
				...
				$this->server->updatePlayerListData($pk->uuid, $pk->eid, $name, $data["skin_id"], base64_decode($data["skin"]),$this->server->getOnlinePlayers());
	...
	public function onTap(PlayerInteractEvent $event)
	{
		if(isset($this->tap[$event->getPlayer()->getName()]))
		{
	        ...
	        if($id === "player")
	        {
			    ...
			    $this->server->updatePlayerListData($pk->uuid, $pk->eid, $n, $player->getSkinId(), $player->getSkinData(),$this->server->getOnlinePlayers());
	...
	public function add($eid, array $options, $player = null)
	{
		...
		if($type === "player")
	    {
			...
			$this->uuidFolder[$pk->eid] = $pk->uuid;
			$this->server->updatePlayerListData($pk->uuid, $pk->eid, $name, $options["skin_id"], base64_decode($options["skin"]),$this->server->getOnlinePlayers());
			...
			if($player !== null)
			{
				...
				$this->server->broadcastPacket($this->server->getOnlinePlayers(), $remove);
			}
